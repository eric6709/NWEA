...
print("NWEA MAP Fall to Spring Growth by Class and Grade for LOWEST QUARTILE")
...
## Remove from fall scores any test results that are not valid growth measures and any scores not in lowest quartile       
fscores <- filter(fscores, GrowthMeasureYN==TRUE)
fscores <- filter(fscores, TestPercentile<25)
...
write.csv(mgrowthbyclass, file="Q1FallToSpringMathGrowthByClass.csv", row.names=FALSE)
write.csv(rgrowthbyclass, file="Q1FallToSpringReadingGrowthByClass.csv", row.names=FALSE)
write.csv(mgrowthbygrade, file="Q1FallToSpringMathGrowthByGrade.csv", row.names=FALSE)
write.csv(rgrowthbygrade, file="Q1FallToSpringReadingGrowthByGrade.csv", row.names=FALSE)
