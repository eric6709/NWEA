...
# Calculates growth ONLY for students in LOWEST QUARTILE of national norms on baseline test.

## Load required R packages
library(dplyr)

print("NWEA MAP Spring to Spring Growth by Class and Grade for LOWEST QUARTILE")
...
print("The program outputs tables with growth data for each class and grade level, including ONLY students scoring below 25th percentile on baseline test.")
...
## Remove from baseline scores any test results that are not valid growth measures and any scores not in lowest quartile       
bscores <- filter(bscores, GrowthMeasureYN==TRUE)
bscores <- filter(bscores, TestPercentile<25)
...
## Summarize the data by class, then add error bands
...
mgrowthbyclass <- mgrowthbyclass[,c(2,1,3,4,5,6,7,8,17,18,10,11,12,13,14,15)]
...
rgrowthbyclass <- rgrowthscores %>% 
        ...
        mutate(PercentofTypicalGrowthLowerBound = (ActualMeanRITGrowth - CohortGrowthME) / TypicalMeanRITGrowth) %>%
        mutate(PercentofTypicalGrowthUpperBound = (ActualMeanRITGrowth + CohortGrowthME) / TypicalMeanRITGrowth)
rgrowthbyclass <- rgrowthbyclass[,c(2,1,3,4,5,6,7,8,17,18,10,11,12,13,14,15)] 
...
mgrowthbygrade <- mgrowthscores %>% 
        ...
        mutate(PercentofTypicalGrowthLowerBound = (ActualMeanRITGrowth - CohortGrowthME) / TypicalMeanRITGrowth) %>%
        mutate(PercentofTypicalGrowthUpperBound = (ActualMeanRITGrowth + CohortGrowthME) / TypicalMeanRITGrowth)
mgrowthbygrade <- mgrowthbygrade[,c(1,2,3,4,5,6,7,16,17,9,10,11,12,13,14)]
...
rgrowthbygrade <- rgrowthscores %>% 
        ...
        mutate(PercentofTypicalGrowthLowerBound = (ActualMeanRITGrowth - CohortGrowthME) / TypicalMeanRITGrowth) %>%
        mutate(PercentofTypicalGrowthUpperBound = (ActualMeanRITGrowth + CohortGrowthME) / TypicalMeanRITGrowth)
rgrowthbygrade <- rgrowthbygrade[,c(1,2,3,4,5,6,7,16,17,9,10,11,12,13,14)] 

## Save tables to files and print completion message
write.csv(mgrowthbyclass, file="Q1SpringToSpringMathGrowthByClass.csv", row.names=FALSE)
write.csv(rgrowthbyclass, file="Q1SpringToSpringReadingGrowthByClass.csv", row.names=FALSE)
write.csv(mgrowthbygrade, file="Q1SpringToSpringMathGrowthByGrade.csv", row.names=FALSE)
write.csv(rgrowthbygrade, file="Q1SpringToSpringReadingGrowthByGrade.csv", row.names=FALSE)
